...
class GoNNet(nn.Module):
    ...
    def __init__(self, game: GoGame, args: DotDict) -> None:
        ######################################
        #        YOUR CODE GOES HERE         #
        ######################################
        super().__init__()
...
class GoNNetWrapper:
    ...
    def train(
        ...
        optimizer: optim.Adam = optim.Adam(params=self.nnet.parameters(), lr=lr)
    ...
    def save_checkpoint(
        self, folder: str | Path = "checkpoint", filename: str = "checkpoint.pth.tar"
    ):
